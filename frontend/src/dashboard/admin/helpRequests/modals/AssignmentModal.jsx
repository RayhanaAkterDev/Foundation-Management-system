...
const AssignmentModal = ({
  ...
  // ============================================================
  // VERIFIED ORGANIZATIONS AVAILABLE FOR THIS REQUEST
  // ============================================================
  const availableOrganizations = useMemo(() => {
        ...
        .filter(
          (assignment) =>
            String(assignment?.status || "")
              .trim()
              .toLowerCase() === "rejected" && assignment?.organization_id,
        )
    ...
    return organizations.filter((organization) => {
      ...
      // An organization that rejected THIS request
      ...
      // Keep existing behavior for organizations
      // where verification status is unavailable.
  ...
  // ============================================================
  // ORGANIZATIONS THAT PREVIOUSLY REJECTED THIS REQUEST
  // ============================================================
  const rejectedOrganizations = useMemo(() => {
    const requestAssignments = Array.isArray(request?.assignments)
      ? request.assignments
      : request?.assignment
        ? [request.assignment]
    ...
    return organizations.filter((organization) => {
      ...
      return organizationId && rejectedOrganizationIds.has(organizationId);
    });
  }, [organizations, request]);
...
const AssignmentForm = ({
  ...
  const selectedOrganizationData = availableOrganizations.find(
    (organization) =>
      String(organization?.id ?? organization?.organization_id ?? "") ===
  ...
  const requestUrgency = request.priority || request.urgency || "Normal";
          ...
          <div className="flex flex-1 flex-col px-7 py-8">
            ...
            <div className="mt-9">
              <div className="border-t border-white/10">
                <InfoRow label="Category" value={requestCategory} />

                <InfoRow label="Urgency" value={requestUrgency} indicator />

                {request.district && (
                  <InfoRow label="Location" value={request.district} icon />
                )}
              </div>
            </div>
              ...
              <div className="border-t border-white/10 pt-6">
                <div className="flex gap-3">
                  ...
                  <p className="text-[11px] leading-[1.65] text-white/50">
                    Assignment requests are sent only to verified organizations
                    eligible to coordinate this request.
                  </p>
                </div>
              </div>
            </div>
          ...
          <header className="relative shrink-0 border-b border-slate-200 px-6 py-6 sm:px-8">
            ...
            <div className="pr-12">
              ...
              <p className="mt-1.5 max-w-xl text-[13px] leading-5 text-slate-500">
                Select a verified organization to coordinate support for this
                request.
              </p>
            </div>
          </header>
          ...
          <form
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-7 sm:px-8 sm:py-8">
              ...
              <section>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 max-w-md text-xs leading-5 text-slate-500">
                            There are currently no verified organizations
                ...
                {/* =================================================
                                    PREVIOUSLY REJECTED ORGANIZATIONS
                                ================================================== */}
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          ...
                          <h4 className="mt-1.5 text-[15px] font-semibold tracking-tight text-slate-800">
                            Rejected organizations
                          </h4>
                        </div>

                        <span className="shrink-0 text-xs text-slate-400">
                          {rejectedOrganizations.length} rejected
                        </span>
                      </div>

                      <p className="mt-1.5 text-xs leading-5 text-slate-500">
                        These organizations have already rejected this request
                        and cannot be assigned again.
                      </p>
                    </div>

                    <div className="space-y-2">
                      {rejectedOrganizations.map((organization) => (
                        <div
                          key={organization.id}
                          className="flex items-center justify-between gap-4 border border-red-100 bg-red-50/60 px-4 py-3.5">
                          <div className="flex min-w-0 items-center gap-3">
                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white text-red-400 ring-1 ring-red-100">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="mt-0.5 text-[11px] text-slate-500">
                                Previously rejected this request
                              </p>
                            </div>
                          </div>

                          <span className="flex shrink-0 items-center gap-1.5 text-[11px] font-semibold text-red-600">
              ...
              {/* Small workflow hint */}
              {availableOrganizations.length > 0 && !hasAssignmentTarget && (
                <div className="mt-7 flex items-start gap-3 border-t border-slate-100 pt-5">
                  ...
                  <p className="text-xs leading-5 text-slate-500">
                    Choose an organization above to send an assignment request.
                    The organization can then respond to the request.
                  </p>
                </div>
              )}
            </div>
            ...
            <footer className="flex shrink-0 items-center justify-between gap-4 border-t border-slate-200 bg-white px-6 py-4 sm:px-8">
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                <button
                  ...
                  className="inline-flex h-10 min-w-42.5 items-center justify-center gap-2 rounded-lg bg-primary px-5 py-2 text-[13px] font-semibold text-white shadow-sm transition-all hover:bg-primary-hover hover:shadow-md disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none">
                  {loading && (
                    <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" />
                  )}

                  {loading ? "Sending..." : "Send assignment request"}
                </button>
              </div>
            </footer>
          </form>
